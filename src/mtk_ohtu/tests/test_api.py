...
def test_api_incorrect_url(client):
    with client:
        response = client.post(
            ...
            content_type="application/json; charset=utf-8",
            headers={"API-Key": "test_api_key"},
            json={"address": {"streetAddress": "Helsinki"}, "posting_id": 6},
        )
...
def test_api_incorrect_parameter_types(client):
    with client:
        response = post_test(
            client,
            json={"address": {"streetAddress": "Turku"}, "posting_id": "test_str"},
        )
        assert response.content_type == "application/json"
        msg = response.json["message"]
...
def test_api_invalid_listing_id(client):
    with client:
        response = post_test(
            client, json={"address": {"streetAddress": "Helsinki"}, "posting_id": -512}
        )
...
def test_api_invalid_address(client):
    with client:
        response = post_test(
            client,
            json={"posting_id": 6, "address": {"streetAddress": "-612361236123"}},
        )
        msg = response.json["message"]
        assert response.status_code == 400
...
def test_api_correct_response(client):
    with client:
        response = post_test(
            client, json={"posting_id": 3, "address": {"streetAddress": "Helsinki"}}
        )
