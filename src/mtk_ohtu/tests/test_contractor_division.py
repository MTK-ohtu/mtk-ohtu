...
def db_get_locations_by_cargo_type(cargo_type, db_connection):
    ...
    for i in range(100):
        ...
        mockList.append(
            LogisticsNode(10, 10, "address", "name", location, random_delivery_radius)
        )
    return mockList
...
def distanceBetween(a: Location, b: Location):
    distance = (
        geopy.distance.geodesic((a.latitude, a.longitude), (b.latitude, b.longitude)).m
        / 1000
    )
    return distance
...
def test_optimal_list_contains_only_contractors_within_two_given_range(setUp):
    for c in division.optimal:
        assert c.delivery_radius >= distanceBetween(
            product_location, c.location
...
def test_feature_collection_contains_viable_features(setUp):
    ...
    assert all(
        type(feature) == Feature for feature in optimal_feature_collection.features
    )
    assert all(
        type(feature) == Feature for feature in suboptimal_feature_collection.features
    )
